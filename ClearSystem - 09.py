...
def get_caminhos_settings(SYSTEM_NAME,TEXT_SEARCHED, TYPE_FILE_TXT):
    ...
    if os.path.exists(SETTINGS_FILE):
        if TYPE_FILE_TXT == 'settings.conf':
            ...
            #print(RETURN_LINE, ' <<<<<< AAAAAAAAAAAAAA')      
            return RETURN_LINE
        if TYPE_FILE_TXT == 'exclude.txt' or TYPE_FILE_TXT == 'include.txt' or TYPE_FILE_TXT == 'menu.txt':
            with open(SETTINGS_FILE, 'r') as arquivo:
                ...
                #print(LINHA_SETTINGS, '11 <<<<<< 11+++++++++++11 >>>>>> 11')
                #print(type(LINHA_SETTINGS), ' <<<<<< +++++++++++ >>>>>> ')
                if LINHA_SETTINGS == 'None' or LINHA_SETTINGS == 'NoneType': 
                    #print(LINHA_SETTINGS, '22 <<<<<< 11+++++++++++11 >>>>>> 22')
                    #print(type(LINHA_SETTINGS), ' <<<<<< +++++++++++ >>>>>> ')
                    LINHA_SETTINGS = []
                    #print(LINHA_SETTINGS, '33 <<<<<< 11+++++++++++11 >>>>>> 33')
                    #print(type(LINHA_SETTINGS), ' <<<<<< +++++++++++ >>>>>> ')
                LINHA_SETTINGS = LINHA_SETTINGS
                #print(LINHA_SETTINGS, '44 <<<<<< 11+++++++++++11 >>>>>> 44')
                #print(type(LINHA_SETTINGS), ' <<<<<< +++++++++++ >>>>>> ')
        #print(LINHA_SETTINGS, ' <<<<<< BBBBBBBBBBBBBBBBB') 
        #print(LINHA_SETTINGS, '55 <<<<<< 11+++++++++++11 >>>>>> 55')
        #print(type(LINHA_SETTINGS), ' <<<<<< +++++++++++ >>>>>> ')
...
for System_Name in INSTALLED_SYSTEMS:
    ...
    for Search_Texto in RETURN_MEDIA_NAME:
        ...
        #print(type(ROM_DIR), 'Type ROM_DIR \n', ' ------ <<^^^|^^^>> ------')
        REAL_FILES = [os.path.splitext(f)[0] for f in os.listdir(ROM_DIR) if os.path.isfile(os.path.join(ROM_DIR, f))]
        #print(type(REAL_FILES), 'Type REAL_FILES \n', ' ------ <<^^^|^^^>> ------')
        REAL_EXT_FILES = [os.path.splitext(f)[1] for f in os.listdir(str(ROM_DIR)) if os.path.isfile(os.path.join(str(ROM_DIR), f))]
        #print(type(REAL_EXT_FILES), 'Type REAL_EXT_FILES \n', ' ------ <<^^^|^^^>> ------')
        EXCLUIR_TXT_LIST = (get_caminhos_settings(System_Name,'', 'exclude.txt'))
        #print(type(EXCLUIR_TXT_LIST), 'Type EXCLUIR_TXT_LIST \n', ' ------ <<^^^|^^^>> ------')
        INCLUDE_TXT_LIST = (get_caminhos_settings(System_Name,'', 'include.txt'))
        #print(type(INCLUDE_TXT_LIST), 'Type INCLUDE_TXT_LIST \n', ' ------ <<^^^|^^^>> ------')
        ROMS_XML_LIST = get_name_rom_xml(System_Name)
        #print(type(ROMS_XML_LIST), 'Type ROMS_XML_LIST \n', ' ------ <<^^^|^^^>> ------')
        DEFAULT_LIST = ['default','EstaESomenteUmaListaFake']
        #print(type(DEFAULT_LIST), 'Type DEFAULT_LIST \n', ' ------ <<^^^|^^^>> ------')
        #if EXCLUIR_TXT_LIST or INCLUDE_TXT_LIST or ROMS_XML_LIST or REAL_FILES is not None :
        if EXCLUIR_TXT_LIST is not None :
            IGNORE_FILE = EXCLUIR_TXT_LIST + INCLUDE_TXT_LIST + DEFAULT_LIST + ROMS_XML_LIST
        if EXCLUIR_TXT_LIST is None :
            IGNORE_FILE = INCLUDE_TXT_LIST + DEFAULT_LIST + ROMS_XML_LIST
        if INCLUDE_TXT_LIST is None :
            IGNORE_FILE = EXCLUIR_TXT_LIST + DEFAULT_LIST + ROMS_XML_LIST
        elif EXCLUIR_TXT_LIST and INCLUDE_TXT_LIST is None :
            IGNORE_FILE = DEFAULT_LIST + ROMS_XML_LIST
        IGNORE_FILE = DEFAULT_LIST + ROMS_XML_LIST
        #   CRIAR LISTA DE ARQUIVOS PARA REMOÇÃO
        tmpw = np.array(REAL_FILES)
        tmpz = np.array(IGNORE_FILE)
...
# ! ----------------------------------------------------------------------------------------------------- ! #
        print(REMOVE_LIST_FINAL, 'print(REMOVE_LIST_FINAL)')
# ! ----------------------------------------------------------------------------------------------------- ! #
